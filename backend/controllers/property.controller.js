...
export const createProperty = async (req, res) => {
    ...
    if ( !property.name || !property.price || !property.image ) {
        return res.status(400).json({ success:false, message: "Por favor rellena todos los campos necesarios." });
    }
...
export const deleteProperty = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ success:true, message: "Propieded eliminada." });
    } catch (error) {
